...
- [x] **Optional `.htaccess` gateway** (`LD_HTACCESS_REQUIRED=1`): the outermost `onRequest` hook refuses to serve any request that has not provably passed an Apache `.htaccess` Basic-Auth layer. Apache injects a shared secret (`X-LD-Gateway`) + the authenticated user (`X-LD-User`) after login; the app verifies the secret in constant time and requires the user on every request — a direct hit to the Node port (no secret) gets 403. It **refuses to boot** if the configured `.htaccess`/`.htpasswd` is missing or not actually protective (no `AuthType Basic` / `Require valid-user` / non-empty `AuthUserFile`), and **locks to 503** if that protection is removed or weakened while running (re-checked every `LD_HTACCESS_RECHECK_MS`). See `.htaccess.example` and installer.md §7b.
