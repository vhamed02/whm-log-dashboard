...
const raw = {
  ...
  notify: {
    ...
    // Entries the test scan reads before it stops. The sample groups identical
    // entries and counts them, so it cannot stop at the first N matches the way
    // an ungrouped sample could — the occurrence count is only meaningful over a
    // known span. This cap is that span, and keeps a test click bounded.
    sampleMaxBlocks: parseInt(process.env.LD_NOTIFY_SAMPLE_MAX_BLOCKS || '2000', 10),
    // Theme re-report scan. A theme problem must keep arriving in EVERY digest
    // for as long as its lines are still in the log file, so each digest re-reads
    // the selected files rather than trusting the watcher's position (which only
    // ever sees bytes appended since it attached). Budgeted separately from the
    // test sample because this runs once per account per cadence, unattended,
    // whereas a test sample runs on a button press.
    themeScanMaxBytesPerFile: parseInt(process.env.LD_NOTIFY_THEME_SCAN_MAX_BYTES_PER_FILE || '4194304', 10),
    themeScanMaxBytesTotal: parseInt(process.env.LD_NOTIFY_THEME_SCAN_MAX_BYTES_TOTAL || '33554432', 10),
    themeScanMaxBlocks: parseInt(process.env.LD_NOTIFY_THEME_SCAN_MAX_BLOCKS || '5000', 10),
