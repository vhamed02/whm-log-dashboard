...
const raw = {
  ...
  logExtra: (process.env.LD_LOG_EXTRA || 'error_log').split(',').map(s => s.trim()).filter(Boolean),
  ...
  // --- .htaccess gateway hardening ---
  // When required=1 the app refuses to serve any request that has not provably
  // passed an Apache .htaccess Basic-Auth layer in front of it: Apache injects a
  // shared secret header + the authenticated user, and this app verifies both on
  // every request. See lib/htaccess.js and .htaccess.example.
  htaccess: {
    required: envBool(process.env.LD_HTACCESS_REQUIRED, false),
    // Shared secret Apache sets on gatewayHeader after a successful login, proving
    // the request came through the .htaccess-protected proxy. Mandatory (>=16
    // chars) when required=1. Never leaves the server pair — treat like a password.
    secret: process.env.LD_HTACCESS_SECRET || '',
    // Optional path to the .htaccess whose protective directives are validated at
    // boot and periodically. When set, a missing/weakened file fails the app safe.
    file: (process.env.LD_HTACCESS_FILE || '').trim(),
    // AuthUserFile (.htpasswd). When empty it is read from the .htaccess itself.
    passwdFile: (process.env.LD_HTPASSWD_FILE || '').trim(),
    // Header names Apache injects. Lower-cased: Node header keys are lower-case.
    gatewayHeader: (process.env.LD_HTACCESS_GATEWAY_HEADER || 'x-ld-gateway').trim().toLowerCase(),
    userHeader: (process.env.LD_HTACCESS_USER_HEADER || 'x-ld-user').trim().toLowerCase(),
    // How often the .htaccess/.htpasswd are re-validated on disk (ms; min 5000).
    recheckMs: parseInt(process.env.LD_HTACCESS_RECHECK_MS || '30000', 10),
  },
