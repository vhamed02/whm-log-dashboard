 ...
 *
 * Flow per interval (default: top of every hour):
 *   for each account with a non-empty buffer -> compose ONE digest -> send to
 *   every selected recipient in a single Brevo call -> clear the buffer.
 *   Accounts that buffered nothing are skipped entirely — a quiet hour sends
 *   no mail at all.
 */
const { config, RECIPIENTS } = require('./config');
...
class Notifier {
  ...
  start() {
    ...
    // Align the first flush to the next interval boundary since the epoch. With
    // the default 1h interval that lands on the top of the hour, so digests
    // arrive at predictable times rather than at "whenever the process booted".
    const now = Date.now();
    const iv = config.notify.intervalMs;
    const firstAt = Math.ceil(now / iv) * iv;
    this.nextFlushAt = firstAt;
    const kickoff = setTimeout(() => {
      this._safeFlush();
      this.flushTimer = setInterval(() => this._safeFlush(), iv);
      if (this.flushTimer.unref) this.flushTimer.unref();
      this.nextFlushAt = Date.now() + iv;
    }, firstAt - now);
    ...
    this.log.info(
      ...
      `, interval=${Math.round(iv / 60000)}min, first flush ${new Date(firstAt).toISOString()}`
    );
  }
  ...
  _onEntry(account, norm) {
    ...
    if (!buf) {
      buf = { groups: new Map(), overflow: 0, total: 0, since: Date.now() };
  ...
  status() {
    ...
    for (const [account, buf] of this.buffers) {
      buffered[account] = { total: buf.total, groups: buf.groups.size, since: buf.since, counts: severityCounts(buf) };
    }
    const watching = {};
  ...
  // Timer-driven entry point. flush() already handles per-account send errors,
  // but anything unexpected escaping it would become an unhandled rejection —
  // which terminates the process on Node 15+. A broken digest must never take
  // the log viewer down with it.
  _safeFlush() {
    this.flush('interval').catch((e) => {
  ...
  /**
   * Send one digest per account that has buffered entries. Accounts with an
   * empty buffer are skipped — a quiet interval produces no email.
   */
  async flush(reason = 'manual') {
    if (this.flushing) return; // a slow Brevo call must not overlap the next tick
    this.flushing = true;
    this.lastFlushAt = Date.now();
    this.nextFlushAt = Date.now() + config.notify.intervalMs;
    const armed = config.notify.enabled;
    try {
      for (const account of [...this.buffers.keys()]) {
        const buf = this.buffers.get(account);
        if (!buf || !buf.total) { this.buffers.delete(account); continue; }

        const s = store.getRaw(account);
        if (!store.isActive(s)) { this.buffers.delete(account); continue; }
        const recips = this.recipientsFor(s);
        if (!recips.length) { this.buffers.delete(account); continue; }

        const digest = composeDigest(account, buf, s);

        if (!armed) {
          this.log.info(
            `[notify] DRY RUN (${reason}) account=${account} — would send "${digest.subject}" to ` +
        ...
        try {
          ...
          this.log.info(`[notify] sent account=${account} to=${recips.map(r => r.email).join(',')} entries=${buf.total} messageId=${id || 'n/a'}`);
          this.buffers.delete(account);
          this.lastError = null;
        } catch (e) {
