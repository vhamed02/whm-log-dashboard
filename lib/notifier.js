...
const state = require('./notify-state');
...
/**
 * WordPress THEME problems are the opposite case to plugins: plugins are muted
 * entirely, themes are the ones that must keep coming back.
 *
 * Every other entry is reported exactly once. That is a property of POSITION,
 * not of content: LiveTail seeds its offset from the file size when it attaches,
 * so it only ever reads bytes appended after that, and the digest buffer is
 * dropped after each send. Nothing anywhere records what was emailed, so nothing
 * can be emailed twice.
 *
 * A theme problem is exempt. It is re-read from the log file itself on every
 * digest and reported again for as long as those lines are present, so an
 * unfixed theme fault keeps arriving instead of scrolling out of one email and
 * being forgotten. Deleting the lines once the fault is fixed is what stops it —
 * that is the intended workflow, and the log file is deliberately the only
 * source of truth for whether a problem is still open.
 *
...
/**
 * Severities that are re-reported. The request was "theme errors or warnings",
 * and critical is included because a theme fatal is the strongest case of all.
 * `info` is excluded: it is the catch-all bucket for unclassified lines, and
 * re-reporting it every cadence would turn the digest into noise.
 */
const THEME_SEVERITIES = new Set(['critical', 'error', 'warning']);
...
/**
 * True when an entry is one that must be re-reported until its lines are gone.
 *
 ...
 */
function isThemeEntry(norm) {
  return THEME_SEVERITIES.has(norm.severity) && isThemePath(norm.message);
}
...
/**
 * Every theme entry still present in the account's log files, grouped the normal
 * way (identical messages collapse into one row carrying an occurrence count).
 *
 * This is the whole re-reporting mechanism. The digest buffer only holds what
 * arrived since the last send, so an entry already emailed can never reappear
 * from it; this re-reads the FILES instead, finds the same lines again, and
 * reports them again. Removing the lines once the fault is fixed is what stops
 * it — the log file is deliberately the only source of truth for "still open".
 *
 * `lastSentAt` splits each group's occurrences into ones the recipient has
 * already seen and ones written since the last email, so the digest can say
 * "already reported" instead of passing a repeat off as a fresh failure.
 *
 * Bounded on every axis, like the test sampler: bytes per file, bytes overall,
 * and entries scanned. This runs unattended once per account per cadence, so an
 * unbounded scan would put a growing cost on every busy box.
 *
 * Returns { groups: Map<key, group>, scanned, capped, files, lastSentAt }.
 */
async function scanThemeGroups(account, settings, lastSentAt = 0) {
  const wanted = new Set((settings.severities || []).filter(sev => THEME_SEVERITIES.has(sev)));
  const groups = new Map();
  if (!wanted.size) return { groups, scanned: 0, capped: false, files: 0, lastSentAt };

  const perFileCap = config.notify.themeScanMaxBytesPerFile;
  let budget = config.notify.themeScanMaxBytesTotal;
  let remaining = Math.max(1, config.notify.themeScanMaxBlocks);
  let scanned = 0;
  let files = 0;
  ...
  for (const rel of notifiableFiles(settings)) {
    ...
    try {
      ...
      for await (const block of gen) {
        ...
        // Plugin exclusion still wins: an entry routed through plugin code is
        // plugin noise even when a theme file appears in it too.
        if (wanted.has(norm.severity) && isThemePath(norm.message) && !isPluginEntry(norm)) {
          const key = groupKey(norm);
          const tsMs = Date.parse(norm.ts) || Date.now();
          const g = groups.get(key);
          if (g) {
            g.count++;
            // Scanning newest-first, so a repeat is normally older than the one
            // already held — but log timestamps are not strictly monotonic, so
            // widen the window in both directions rather than assuming.
            if (tsMs < g.first) g.first = tsMs;
            if (tsMs > g.last) g.last = tsMs;
            if (tsMs > lastSentAt) g.newSince++;
          } else if (groups.size < config.notify.maxGroups) {
            groups.set(key, {
    ...
    } catch { /* unreadable file — the others still scan */ }
  }
  return { groups, scanned, capped: remaining <= 0, files, lastSentAt };
}

/**
 * Combine the live buffer with the theme rescan into the set of groups an email
 * should show.
 *
 * The scan re-reads the file, so for a theme message it normally already
 * contains everything the buffer caught this window plus the older occurrences
 * the buffer never saw — which is why it wins. The buffer's copy is kept only
 * when it somehow holds MORE, which happens when the lines were rotated or
 * deleted between arriving and the digest being built: without this the entry
 * would vanish from the email it was supposed to be in.
 */
function mergeGroups(bufGroups, themeGroups) {
  const out = new Map(bufGroups);
  for (const [key, g] of themeGroups) {
    const buffered = out.get(key);
    if (!buffered || g.count >= buffered.count) out.set(key, g);
  }
  return out;
}

/**
 * Label a scanned theme group by what the recipient has already seen, so a
 * repeat never reads as a fresh failure:
 *   all     — every occurrence predates the last email; nothing new happened.
 *   partial — already reported, but it has fired again since.
 * A group with no `lastSentAt` (nothing ever sent) or one that is entirely new
 * carries no label at all.
 */
function markRepeat(g, lastSentAt) {
  if (!lastSentAt || !g.fromLog) return g;
  const fresh = g.newSince || 0;
  if (fresh >= g.count) return g;
  return { ...g, repeat: fresh > 0 ? { kind: 'partial', fresh } : { kind: 'all' } };
}

/**
 * Build the digest email. Pure — does not mutate or clear the buffer or the
 * scan, so /notify/preview can call it safely at any time.
 *
 * `buf` may be null: an account with nothing new still gets a digest when the
 * rescan finds theme problems that are still open in its logs.
 */
function composeDigest(account, buf, settings, theme = null) {
  const bufGroups = buf ? buf.groups : new Map();
  const lastSentAt = theme ? theme.lastSentAt : 0;
  const merged = theme ? mergeGroups(bufGroups, theme.groups) : new Map(bufGroups);
  const all = [...merged.values()].map(g => markRepeat(g, lastSentAt));

  const counts = {};
  let total = 0;
  for (const g of all) { counts[g.severity] = (counts[g.severity] || 0) + g.count; total += g.count; }
  const order = Object.keys(counts).sort((a, b) => SEV_RANK[a] - SEV_RANK[b]);
  const summary = order.map(sev => `${counts[sev]} ${sev}`).join(', ') || 'no entries';
  ...
  const since = buf ? buf.since : (lastSentAt || Date.now());
  const windowLine = `${fmtTime(since)} → ${fmtTime(Date.now())}`;

  const repeated = shown.filter(g => g.repeat).length;

  const notes = [];
  if (hiddenGroups > 0) notes.push(`+${hiddenGroups} more distinct message${hiddenGroups === 1 ? '' : 's'} not shown.`);
  if (buf && buf.overflow) notes.push(`${buf.overflow} further entr${buf.overflow === 1 ? 'y' : 'ies'} were not grouped (per-digest group cap of ${config.notify.maxGroups} reached).`);
  if (repeated > 0) {
    notes.push(`${repeated} theme message${repeated === 1 ? ' is' : 's are'} marked "already reported": ${repeated === 1 ? 'it is' : 'they are'} still present in the log and will be reported in every digest until those lines are removed.`);
  }
  if (theme && theme.capped) {
    notes.push(`The theme re-check stopped after the newest ${theme.scanned} entries in each selected file; anything older than that is not counted.`);
  }
  ...
  return { subject, html, text, counts, total, groups: groups.length, repeated };
}
    ...
    try {
      ...
      for await (const block of gen) {
        ...
        if (wanted.has(norm.severity) && !isPluginEntry(norm)) {
          ...
          if (g) {
            // Scanning newest-first, so a repeat is normally older than the one
            // already held — but timestamps in a log are not strictly monotonic,
            // so widen the window in both directions rather than assuming.
            g.count++;
            if (tsMs < g.first) g.first = tsMs;
            if (tsMs > g.last) g.last = tsMs;
...
async function composeTest(account, settings, perSeverity = 5) {
  ...
  // The chips and the subject count OCCURRENCES; `groups` is how many rows show.
  ...
  const { html, text } = renderEmail({
    ...
    subject,
    windowLine: found
      ? `Newest ${perSeverity} distinct message${perSeverity === 1 ? '' : 's'} per severity, repeats grouped — sampled from ${sampled.length} watched file${sampled.length === 1 ? '' : 's'}`
...
class Notifier {
  ...
  // Clears an account's buffered entries WITHOUT unscheduling it. The schedule
  // is owned by sync()/_tick() now, because an account with an empty buffer must
  // still be visited every cadence to re-check its logs for open theme faults.
  _dropBuffer(account) {
    this.buffers.delete(account);
  }

  /** Forget an account entirely — buffer, schedule and last-sent record. */
  _forgetAccount(account) {
    this.buffers.delete(account);
  ...
  sync() {
    ...
    // Every ACTIVE account is scheduled, whether or not anything has been
    // buffered for it. Before theme re-reporting the schedule was created by the
    // first entry to arrive, because an account with nothing buffered had
    // nothing to send; now it does — a theme fault still sitting in its log is
    // reported again every cadence, and an account whose logs are quiet is
    // exactly the one that needs the re-check.
    const now = Date.now();
    for (const [account, s] of Object.entries(all)) {
      if (!store.isActive(s)) { this._forgetAccount(account); continue; }
      if (!this.dueAt.has(account)) {
        const iv = periodToMs(s.period);
        this.dueAt.set(account, { at: nextBoundaryAfter(now, iv), iv });
      }
    }
    for (const account of [...this.dueAt.keys()]) {
      if (!all[account] || !store.isActive(all[account])) this._forgetAccount(account);
    }
  }
  ...
  // Async now: the preview runs the same theme rescan the real digest does, so
  // what is shown on screen is what would actually be sent — including the
  // still-open theme faults that are not in the buffer at all.
  async preview(account) {
    const buf = this.buffers.get(account);
    const s = store.get(account);
    let theme = null;
    try { theme = await scanThemeGroups(account, s, state.getLastSentAt(account)); } catch { /* preview what we have */ }
    const hasBuffered = !!(buf && buf.total);
    const hasTheme = !!(theme && theme.groups.size);
    if (!hasBuffered && !hasTheme) return { empty: true, settings: s, recipients: this.recipientsFor(s) };
    return {
      empty: false,
      settings: s,
      recipients: this.recipientsFor(s),
      ...composeDigest(account, buf || null, s, theme),
    };
  }
  ...
  _tick() {
    ...
    const due = [];
    // Driven by the schedule rather than by which accounts happen to have
    // buffered something: an account with an empty buffer may still owe a digest
    // for theme faults that are still open in its log files.
    for (const account of [...this.dueAt.keys()]) {
      const iv = this._periodMs(account);
      const d = this.dueAt.get(account);
      if (!d || d.iv !== iv) {
  ...
  async flush(reason = 'manual', only = null) {
    ...
    try {
      // Candidates come from the schedule, not the buffers: an account with
      // nothing buffered may still owe a digest for theme faults still open in
      // its logs.
      const accounts = only ? [...only] : [...this.dueAt.keys()];
      for (const account of accounts) {
        ...
        const buf = this.buffers.get(account);
        // Re-read the log files for theme problems that are still present. This
        // is what makes an unfixed theme fault arrive again in every digest; a
        // send failure or a restart cannot lose it, because the source of truth
        // is the file, not anything we are holding in memory.
        let theme;
        try {
          theme = await scanThemeGroups(account, s, state.getLastSentAt(account));
        } catch (e) {
          // A failed rescan must not cost the account its digest — send what the
          // buffer holds and say so.
          this.log.error(`[notify] theme re-check failed account=${account}: ${e.message}`);
          theme = null;
        }

        const hasBuffered = !!(buf && buf.total);
        const hasTheme = !!(theme && theme.groups.size);
        // A cadence with nothing new AND no open theme fault still sends nothing.
        if (!hasBuffered && !hasTheme) { this._dropBuffer(account); continue; }

        const digest = composeDigest(account, buf || null, s, theme);

        if (!armed) {
          this.log.info(
            `[notify] DRY RUN (${reason}) account=${account} cadence=${s.period} — would send "${digest.subject}" to ` +
            `${recips.map(r => r.email).join(', ')} (${digest.total} entries, ${digest.groups} groups, ` +
            `${digest.repeated} already-reported theme message${digest.repeated === 1 ? '' : 's'}). ` +
        ...
        try {
          ...
          this.log.info(
            `[notify] sent account=${account} cadence=${s.period} to=${recips.map(r => r.email).join(',')} ` +
            `entries=${digest.total} repeated=${digest.repeated} messageId=${id || 'n/a'}`
          );
          this._dropBuffer(account);
          // Recorded only after a delivery actually succeeded, so an entry is
          // never labelled "already reported" on the strength of an email the
          // recipient never got. A failed write only costs the next digest its
          // labels; it can never stop mail.
          state.setLastSentAt(account, Date.now()).catch((e) =>
            this.log.error(`[notify] could not persist last-sent time for ${account}: ${e.message}`));
          this.lastError = null;
        } catch (e) {
...
module.exports = {
  ...
  isThemePath, isThemeEntry, scanThemeGroups, mergeGroups, markRepeat,
  THEME_PATH_RE, THEME_SEVERITIES, MAX_RENDERED_GROUPS,
};
