...
async function composeTest(account, settings, perSeverity = 5) {
  ...
  const { html, text } = renderEmail({
    ...
    subject,
    windowLine: found
      ? `Newest ${perSeverity} distinct message${perSeverity === 1 ? '' : 's'} per severity, repeats grouped — sampled from ${settings.files.length} watched file${settings.files.length === 1 ? '' : 's'}`
