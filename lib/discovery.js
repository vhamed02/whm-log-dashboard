...
// Directories that live directly under /home but are never cPanel accounts and
// must never appear in the account list. `virtfs` in particular is cPanel's
// jailshell bind-mount tree (/home/virtfs/<user>/...) — descending into it would
// also duplicate every account's logs — so it is always excluded. The others are
// standard skeleton/system dirs. (Names with characters validAccount() already
// rejects, e.g. `lost+found`, never reach here, but listing them is harmless.)
const RESERVED_HOME_DIRS = new Set(['virtfs', 'cpanel3-skel', 'lost+found', 'quarantine']);
...
async function isCpanelUser(dirPath, name) {
  ...
  // Reserved system/bind-mount dirs under /home that are never accounts.
  if (RESERVED_HOME_DIRS.has(name)) return false;
  try {
